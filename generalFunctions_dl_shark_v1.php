...
function TripCollectTip($iMemberId, $iTripId, $fAmount) {
    ...
    $vSenangToken = get_value($tbl_name, 'vSenangToken', $iUserId, $iMemberId, '', 'true');
...
function ChargeCustomer($Data, $eChargeEvent = "CollectPayment") {
    ...
    /* Added By PM On 09-12-2019 For Flutterwave Code End */
    $vAdyenChargePrice = $vSenangChargePrice = $price_new;
    if ($UserType == "Passenger") {
        $tbl_name = "register_user";
        $iUserId = "iUserId";
        /* Updated By PM On 09-12-2019 For Flutterwave Code Start */
        $UserDetailPaymaya = get_value($tbl_name, 'vPaymayaCustId,vPaymayaToken,vAdyenToken,vSenangToken,vName,vLastName,vEmail,vPhone,vXenditAuthId,vXenditToken,vFlutterWaveToken,vCurrencyPassenger as vCurrency,vLang', $iUserId, $iMemberId);
        /* Updated By PM On 09-12-2019 For Flutterwave Code end */
    } else {
        $tbl_name = "register_driver";
        $iUserId = "iDriverId";
        /* Updated By PM On 09-12-2019 For Flutterwave Code Start */
        $UserDetailPaymaya = get_value($tbl_name, 'vPaymayaCustId,vPaymayaToken,vAdyenToken,vSenangToken,vName,vLastName,vEmail,vPhone,vXenditAuthId,vXenditToken,vFlutterWaveToken,vCurrencyDriver as vCurrency,vLang', $iUserId, $iMemberId);
        /* Updated By PM On 09-12-2019 For Flutterwave Code end */
    }
 
    if ($APP_PAYMENT_METHOD == "Senangpay") {
        //echo "<pre>";print_r($UserDetailPaymaya);die;
        if (isset($UserDetailPaymaya[0]['vSenangToken']) && trim($UserDetailPaymaya[0]['vSenangToken']) != "") {
            //ini_set('display_errors', 1);
            //error_reporting(E_ALL);
            //echo "<pre>";print_r($UserDetailPaymaya);die;
            $vSenangToken = trim($UserDetailPaymaya[0]['vSenangToken']);
            ...
            $hashvar = $SENANGPAY_MERCHANT_ID . $shopperReference . $shopperEmail . $vPhone . $detail . $order_id . $vSenangChargePrice;
            $sig = hash_hmac('sha256', $hashvar, $SENANGPAY_SECRETKEY);
            $postdata = array('name' => $shopperReference, 'email' => $shopperEmail, 'detail' => $detail, 'phone' => $vPhone, 'order_id' => $order_id, 'amount' => $vSenangChargePrice, 'token' => $vSenangToken, 'hash' => $sig);
            ...
            if (isset($result['status']) && $result['status'] == 1) {
                $resultCode = $result['status']; //Authorised
                $transactionId = $result['transaction_id'];
                $Adyen_arr['SENANGPAY_MERCHANT_ID'] = $SENANGPAY_MERCHANT_ID;
                $Adyen_arr['SENANGPAY_SECRETKEY'] = $SENANGPAY_SECRETKEY;
                $Adyen_arr['SENANGPAY_GENERATE_TOKEN_URL'] = $SENANGPAY_GENERATE_TOKEN_URL;
                $Adyen_arr['SENANGPAY_GETPAYMENT_BY_TOKEN_URL'] = $SENANGPAY_GETPAYMENT_BY_TOKEN_URL;
                $tPaymentDetails = json_encode($Adyen_arr, JSON_UNESCAPED_UNICODE);
                $pay_data['tPaymentUserID'] = $transactionId;
                ...
                $msgTxt = "LBL_CHARGE_COLLECT_FAILED";
                if ($eChargeEvent == "addMoneyUserWallet") {
                    $msgTxt = "LBL_WALLET_MONEY_CREDITED_FAILED";
                }
                //print_r($result);die;
                if (isset($result['msg']) && trim($result['msg']) != "") {
                    $msgTxt = trim($result['msg']);
                    $returnArr['message'] = $msgTxt;
                }
