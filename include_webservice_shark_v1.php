...
include_once ('include_generalFunctions_shark_v1.php');
...
function getPassengerDetailInfo($passengerID, $cityName = "", $LiveTripId = "") {
    ...
    if (count($row) > 0) {
        ...
        /* Added By PM On 09-12-2019 For senang Code End */

        if (!empty($vSystemDefaultCurrencyName) && !empty($vSystemDefaultCurrencySymbol)) {
            $row[0]['vSenangpayCurrency'] = $vSystemDefaultCurrencyName;
            $vSenangpayvSymbol = $vSystemDefaultCurrencySymbol;
        }else{
            $defaultCurrencyDataArr = get_value('currency', 'vName,vSymbol', 'eDefault', 'Yes');
            /* Added By PM On 09-12-2019 For Flutterwave Code Start */
        ...
        /* Added By PM On 09-12-2019 For senang Code Start */
        /*if(isset($langLabels['LBL_SENANG_CHARGE_MSG_TX']) && trim($langLabels['LBL_SENANG_CHARGE_MSG_TX']) != ""){
            $LBL_SENANG_CHARGE_MSG_TX = $langLabels['LBL_SENANG_CHARGE_MSG_TX'];
        }else{
            $db_label_senang = $obj->MySQLSelect("SELECT * FROM `language_label` WHERE vLabel = 'LBL_SENANG_CHARGE_MSG_TX' AND vCode = '" . $vLanguage . "'");
...
function getDriverDetailInfo($driverId, $fromSignIN = 0) {
    ...
    if (count($Data) > 0) {
        ...
        //Added By HJ On 17-06-2020 For Optimization currency Table Query End
         ...
         if (!empty($vSystemDefaultCurrencyName) && !empty($vSystemDefaultCurrencySymbol)) {
            $Data[0]['vSenangpayCurrency'] = $vSystemDefaultCurrencyName;
            $vSenangpayvSymbol = $vSystemDefaultCurrencySymbol;
        }else{
...
if ($type == "ChargePassengerOutstandingAmount") {
    ...
    /* Added By PM On 09-12-2019 For Flutterwave Code Start */
    $riderData = $obj->MySQLSelect("SELECT ru.vSenangToken,ru.vStripeCusId,ru.vStripeToken,ru.vCurrencyPassenger,ru.vBrainTreeToken,ru.vPaymayaCustId,ru.vFlutterWaveToken,ru.vPaymayaToken,ru.vXenditToken, ru.vCurrencyPassenger, ru.vLang, cu.vSymbol, cu.Ratio, co.eUnit FROM register_user as ru, currency as cu, country as co WHERE ru.iUserId='" . $iMemberId . "' AND cu.vName = ru.vCurrencyPassenger AND co.vCountryCode = ru.vCountry");
    /* Added By PM On 09-12-2019 For Flutterwave Code End */
