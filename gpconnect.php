	...
	try {
		...
		if($_SESSION['fb_user'] == 'rider') {
			...
			if(count($db_user) > 0){
				...
				$id = $generalobj->encrypt($db_user[0]['iUserId']);
				...
				$sql = "UPDATE register_user set  vImgName='".$imgname."',eSignUpType = 'Google', vPassword_token = '".$newToken."' WHERE iUserId='".$iUserId."'";
				$obj->sql_query($sql); 
				return $generalobj->Checkverification_mobile($db_user[0]['iUserId'],'rider');
			}
			...
			if(count($db_user) > 0){			
				...
				$id = $generalobj->encrypt($db_user[0]['iDriverId']);
				$newToken = $generalobj->RandomString(32);
				$url = $tconfig["tsite_url"].'reset_password.php?type='.$type.'&id='.$id.'&_token='.$newToken;					
