...
$kiosk_section = $generalobj->checkOtherLangDataExist(json_decode($ride_data[0]['lKioskSection'],true),$vCode,$inner_key);
//echo "<pre>";print_r($ride_data[0]['lBannerSection']);die;
...
$menutitleKiosk = !empty($kiosk_section['menu_title_'.$vCode]) ? $kiosk_section['menu_title_'.$vCode] : $kiosk_section['title_'.$vCode];
...
<head>
    ...
    <title><?=$SITE_NAME?> | <?= $langage_lbl['LBL_RIDE'] ?></title>
    <!--
...
<section class="banner-section taxi-app">
    <div class="tab-row-holding">
        <ul class="tab-row">
            ...
            <li data-src="7" class="tab">
                <a href="#booktaxi"><?= $menutitleCall ?></a>
            </li>
            <!-- <li class="tab mob">
                <a href="#">&nbsp</a>
            </li> -->
            ...
            <?php for ($i=1; $i <= 6; $i++) { ?>
                <?php if (!empty($how_it_work_section['hiw_title'.$i.'_'.$vCode]) && !empty($how_it_work_section['hiw_desc'.$i.'_'.$vCode])) { ?>
                    <li data-number="<?php echo $i; ?>">
                        ...
                        <img alt="" class="proc_ico" src="<?= $tconfig["tsite_url"].'resizeImg.php?w=96&src='.$tconfig["tsite_upload_apptype_page_images"].$template.'/'.$how_it_work_section['hiw_img'.$i.'_'.$vCode];?>" xss="removed">  
...
<section class="safety-section taxi-variant page-section" id="fare-estimate">
    <div class="safety-section-inner">
        ...
        <div class="safety-section-right">
            ...
            <form name="_fare_estimate_form" id="_fare_estimate_form" method="post" action="user-fare-estimate" class="gen-from">
...
<!-- *************safty-section section start************* -->
<section class="safety-section taxi-variant page-section for-kiosk" id="kiosk">
    <div class="safety-section-inner">
        <div class="safety-section-left">
            <!-- <div class="safty-image-hold" style="background-image:url(<?php  echo $tconfig["tsite_upload_apptype_page_images"].$template.'/'.$kiosk_section['img_'.$vCode]; ?>)"></div> -->
            <div class="safty-image-hold" style="background-image:url(<?= $tconfig["tsite_url"].'resizeImg.php?h=491&src='.$tconfig["tsite_upload_apptype_page_images"].$template.'/'.$kiosk_section['img_'.$vCode]; ?>)"></div>
        </div>
        <div class="safety-section-right">
            <h3><?php echo $kiosk_section['title_'.$vCode];?></h3>
            <?php echo $kiosk_section['desc_'.$vCode];?>
        </div>
    </div>
</section>
<!-- *************safty-section section end************* -->
<!-- ************* cartype section section start ************* -->
<section class="cartype page-section white-bg" id="our-vehicles">
    <div class="cartype-inner">
        <h3><?php echo $cartype_section['title_'.$vCode];?></h3>
        <strong><?php echo $cartype_section['desc_'.$vCode];?></strong>

        <ul>
        <?php for ($i=1; $i <= 6; $i++) { ?>
            <?php if (!empty($cartype_section['cartype_title'.$i.'_'.$vCode]) && !empty($cartype_section['cartype_img'.$i.'_'.$vCode])) { ?>
                <li data-number="<?php echo $i; ?>">
                <img alt="" src="<?php echo $tconfig["tsite_upload_apptype_page_images"].$template.'/'.$cartype_section['cartype_img'.$i.'_'.$vCode];?>" xss="removed">
                    <strong><?php echo $cartype_section['cartype_title'.$i.'_'.$vCode];?></strong> 
                </li>
            <?php } ?>
        <?php } ?>
        </ul>
...
<script>
	...
	// });
	  $(document).ready(function () {
		...
		$('#to').keyup(function (e) {
			buildAutoComplete("to",e, "<?=$MIN_CHAR_REQ_GOOGLE_AUTO_COMPLETE;?>","<?=$_SESSION['sess_lang'];?>"); 
		});
	});
