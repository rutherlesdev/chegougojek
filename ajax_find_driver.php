...
if($iCompanyId != '')
{
	...
	if($APP_TYPE != 'UberX'){
	$sql = "select iDriverId,vName,vLastName from register_driver where iCompanyId = '".$iCompanyId."' and eStatus != 'Deleted' order by vName ASC";
	
	
	}else{
	
	$sql = "select iDriverId,vName,vLastName from register_driver where iCompanyId = '".$iCompanyId."' and eStatus != 'Deleted' And iDriverVehicleId = '0' order by vName ASC"; 
