...
include_once ('generalFunctions_dl_shark_v1.php');
...
function getPassengerDetailInfo($passengerID, $cityName = "", $LiveTripId = "") {
    ...
    if (count($row) > 0) {
        ...
        /* Added for senangpay Code Start */
        $row[0]['vSenangpayCurrency'] = $defaultCurrencyDataArr[0]['vName'];
        $vSenangpayvSymbol = $defaultCurrencyDataArr[0]['vSymbol'];
        /* Addedfor senangpay Code End */
        ...
        ## Display senangpay Charge Message ##
    /*    $sql = "SELECT * FROM `language_label` WHERE vLabel = 'LBL_SENANG_CHARGE_MSG_TX' AND vCode = '" . $vLanguage . "'";
        $db_label_adyen = $obj->MySQLSelect($sql);
        $LBL_SENANG_CHARGE_MSG_TX = $db_label_adyen[0]['vValue'];

        $amountDataArr = $generalobj->getSupportedCurrencyAmt($row[0]['SENANG_CHARGE_AMOUNT'], $row[0]['vSenangpayCurrency']);
        $row[0]['vSenangpayCurrency'] = $amountDataArr['CURRENCY_CODE'];

        $SENANG_CHARGE_AMOUNT = $amountDataArr['AMOUNT'];

        $SENANG_CHARGE_AMOUNT_USER_ARR = $SENANG_CHARGE_AMOUNT;
        $SENANG_CHARGE_AMOUNT_USER = $vSenangpayvSymbol . $SENANG_CHARGE_AMOUNT;
        $msg = str_replace('####', $SENANG_CHARGE_AMOUNT_USER, $LBL_SENANG_CHARGE_MSG_TX);
        $row[0]['SENANG_CHARGE_AMOUNT'] = $SENANG_CHARGE_AMOUNT;
        $row[0]['SENANG_CHARGE_MESSAGE'] = $msg;*/
        /* Added for senangpay Code Start */
...
function getDriverDetailInfo($driverId, $fromSignIN = 0) {
    ...
    if (count($Data) > 0) {
        ...
        /* Added For senang Code Start */
        $Data[0]['vSenangpayCurrency'] = $defaultCurrencyDataArr[0]['vName'];
        $vSenangpayvSymbol = $defaultCurrencyDataArr[0]['vSymbol'];
        /* Added For senang Code end */
        ...
        /* Added For senangpay Code Start */
       /* $SENANG_CHARGE_AMOUNT = $generalConfigPaymentArr['SENANG_CHARGE_AMOUNT'];

        $sql = "SELECT * FROM `language_label` WHERE vLabel = 'LBL_SENANG_CHARGE_MSG_TX' AND vCode = '" . $vLanguage . "'";
        $db_label_senang = $obj->MySQLSelect($sql);
        $LBL_SENANG_CHARGE_MSG_TX = $db_label_senang[0]['vValue'];

        $amountDataArr = $generalobj->getSupportedCurrencyAmt($Data[0]['SENANG_CHARGE_AMOUNT'], $Data[0]['vSenangpayCurrency']);
        $Data[0]['vSenangpayCurrency'] = $amountDataArr['CURRENCY_CODE'];

        $SENANG_CHARGE_AMOUNT = $amountDataArr['AMOUNT'];

        $SENANG_CHARGE_AMOUNT_USER = $vSenangpayvSymbol . $SENANG_CHARGE_AMOUNT;
        $Data[0]['SENANG_CHARGE_AMOUNT'] = $SENANG_CHARGE_AMOUNT;
        $msg = str_replace('####', $SENANG_CHARGE_AMOUNT_USER, $LBL_SENANG_CHARGE_MSG_TX);
        $Data[0]['SENANG_CHARGE_MESSAGE'] = $msg;*/
        /* Added For senangpay Code Start */
...
if ($type == "addMoneyUserWallet") {
    ...
    /* Added By PM On 09-12-2019 For Flutterwave Code Start */
    $UserCardData = get_value($tbl_name, 'vSenangToken,vStripeCusId,vStripeToken,vBrainTreeToken,vPaymayaCustId,vXenditToken,vFlutterWaveToken,vPaymayaToken,' . $currencycode . ' as currencycode', $iUserId, $iMemberId);
    /* Added By PM On 09-12-2019 For Flutterwave Code End */
...
if ($type == "CheckCard") {
    ...
    } else if ($APP_PAYMENT_METHOD == "Senangpay") {
        if ($UserType == 'Driver') {
            $vSenangToken = get_value('register_driver', 'vSenangToken', 'iDriverId', $iUserId, '', 'true');
        } else {
            $vSenangToken = get_value('register_user', 'vSenangToken', 'iUserId', $iUserId, '', 'true');
        }
...
if ($type == "CaptureCardPaymentOrder") {
    ...
    $IsChargeCustomer = "No";
    //echo "<pre>";print_r($_REQUEST);die;
     if (($vSenangToken == "" || $vSenangToken == NULL) && $APP_PAYMENT_METHOD == "Senangpay") {
        if (!empty($iUserId)) {
            $Squery = "SELECT vSenangToken,vCreditCard FROM register_user WHERE iUserId = '" . $iUserId . "'";
    ...
    if ($ePaymentOption == "Card") {
        ...
        if ($fNetTotal > 0 && $payStatus == "") {
            if ($IsChargeCustomer == "Yes") {
                ...
                if (strtolower($APP_PAYMENT_METHOD) == "senang") {
                    $getWayUrl = $tconfig['tsite_url'] . "senang_register.php?iUserId=" . $iUserId . "&iOrderId=" . $iOrderId . "&amount=" . $price_new . "&ccode=" . $currencyCode . "&userAmount=" . $REFERRAL_AMOUNT_USER . "&vOrderNo=" . $vOrderNo . $extraPara;
                } else {
