<?php
	Class General_dashboard
	{
		...
		public function getCompanycount()
		{
			...
			$sql = "SELECT count(iCompanyId) tot_company FROM company WHERE eStatus != 'Deleted' $eSystem $cmp_ssql ";
		...
		public function getTripStatescount($tripStatus=NULL,$startDate="",$endDate="")
		{
			...
			if($tripStatus != "") {
				...
				$sql = "SELECT count(iTripId) tot_trip FROM trips WHERE 1 = 1 AND eSystem = 'General'".$cmp_ssql.$ssl.$dsql.$location_where.$etypeSql;
				$data = $obj->MySQLSelect($sql);
			}
			return $data;
		}
		...
		public function getTripDateStates($time)
		{
			...
			$sql = "SELECT count(iTripId) as total FROM trips WHERE 1 = 1 AND eSystem = 'General' ".$ssl.$cmp_ssql.$location_where.$etypeSql;
			$data = $obj->MySQLSelect($sql);
			return $data[0]['total'];
		}
		...
		public function getTripAmount($startDate,$endDate,$payment_method="")
		{
			...
			$sql = "SELECT iTripId, fTripGenerateFare, fDiscount,fWalletDebit, iFare, vTripPaymentMode FROM trips WHERE 1 = 1 AND eSystem = 'General' ".$cmp_ssql.$ssl.$dsql . $psql .$locations_where . $etypeSql;
			$data = $obj->MySQLSelect($sql);
