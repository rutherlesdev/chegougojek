...
if(!$userObj->hasPermission('view-location-wise-fare')){
  $userObj->redirect();
}
...
if($keyword != ''){
    if($option != '') {
		...
		if($eStatus != ''){
            $ssql.= " AND ".stripslashes($option)." LIKE '%".$generalobjAdmin->clean($keyword)."%' AND ls.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        }else {
            $ssql.= " AND ".stripslashes($option)." LIKE '%".$generalobjAdmin->clean($keyword)."%' AND ls.eStatus != 'Deleted'";
        }
		 
    }else {
		if($eStatus != ''){
            $ssql.= " AND (lm1.vLocationName LIKE '%".$keyword."%' OR lm2.vLocationName LIKE '%".$keyword."%' OR ls.fFlatfare LIKE '%".$keyword."%' OR vt.vVehicleType LIKE '%".$keyword."%') AND ls.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        }else {
		...
		if($eStatus != ''){
            $ssql.= " AND ls.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        }else{
...
$sql = "SELECT COUNT(ls.iLocatioId) as Total FROM `location_wise_fare` ls left join location_master lm1 on ls.iToLocationId = lm1.iLocationId left join location_master lm2 on ls.iFromLocationId = lm2.iLocationId left join vehicle_type as vt on vt.iVehicleTypeId=ls.iVehicleTypeId WHERE 1 = 1 $eStatussql $ssql";
...
if ($page <= 0)
    $page = 1;
//Pagination End
$sql = "SELECT ls.*,lm1.vLocationName as vToname,lm2.vLocationName as vFromname,vt.vVehicleType FROM `location_wise_fare` ls left join location_master lm1 on ls.iToLocationId = lm1.iLocationId left join location_master lm2 on ls.iFromLocationId = lm2.iLocationId left join vehicle_type as vt on vt.iVehicleTypeId=ls.iVehicleTypeId WHERE 1 = 1 $eStatussql $ssql $ord LIMIT $start, $per_page";
...
<html lang="en">
    ...
    <head>
        <meta charset="UTF-8" />
       <title><?=$SITE_NAME?> | Location Wise Fare</title>
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <select name="option" id="option" class="form-control">
                                      ...
                                      <option value="lm2.vLocationName" <?php if ($option == "lm2.vLocationName") { echo "selected"; } ?> >Source Location</option>
                                      <option value="lm1.vLocationName" <?php if ($option == "lm1.vLocationName") { echo "selected"; } ?> >Destination Location</option>
                                      <option value="ls.fFlatfare" <?php if ($option == "ls.fFlatfare") { echo "selected"; } ?> >Flat Fare</option>
                                    <option value=" vt.vVehicleType" <?php if ($option == "vt.vVehicleType") { echo "selected"; } ?> >Vehicle Type</option>
                                    </select>
                                    </td>
                                    ...
                                    <td width="12%">
                                      ...
                                      <input type="button" value="Reset" class="btnalt button11" onClick="window.location.href='locationwise_fare.php'"/>
                                    </td>
                                    <?php if($userObj->hasPermission('create-location-wise-fare')){ ?>
                                      <td width="20%"><a class="add-btn" href="location_wise_fare_action.php" style="text-align: center;">Add A Location</a></td>
                                    <?php } ?>
                                </tr>
                              </tbody>
                        </table>
                            ...
                            <div class="col-lg-12">
                                <div class="admin-nir-export">
                                    ...
                                    <span class="col-lg-2 new-select001">
                                      <?php if($userObj->hasPermission(['update-status-location-wise-fare', 'delete-location-wise-fare'])){ ?>
                                            <select name="changeStatus" id="changeStatus" class="form-control" onchange="ChangeStatusAll(this.value);">
                                              <option value="" >Select Action</option>
                                              <?php if($userObj->hasPermission('update-status-location-wise-fare')){ ?>
                                              ...
                                              <?php } ?>
                                              <?php if($userObj->hasPermission('delete-location-wise-fare')){ ?>
...
<!--                                    <div class="panel-heading">
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
														...
														<th width="20%"><a href="javascript:void(0);" onClick="Redirect(2,<?php if($sortby == '2'){ echo $order; }else { ?>0<?php } ?>)">Source Location Name<?php if ($sortby == 2) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>

                                                        <th width="20%"><a href="javascript:void(0);" onClick="Redirect(1,<?php if($sortby == '1'){ echo $order; }else { ?>0<?php } ?>)">Destination Location Name<?php if ($sortby == 1) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>                                                   

                                                        <th width="20%"><a href="javascript:void(0);" onClick="Redirect(3,<?php if($sortby == '3'){ echo $order; }else { ?>0<?php } ?>)">Flat Fare<?php if ($sortby == 3) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                       														
                                                         <th width="20%"><a href="javascript:void(0);" onClick="Redirect(5,<?php if($sortby == '5'){ echo $order; }else { ?>0<?php } ?>)">Vehicle Type<?php if ($sortby == 5) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="gradeA">
                                                        ...
                                                        <td><?= $data_drv[$i]['vToname']; ?></td>
                                                        <td><?= $data_drv[$i]['fFlatfare']; ?></td>
                                                        ...
                                                        <td align="center" style="text-align:center;" class="action-btn001">
                                                            <div class="share-button openHoverAction-class" style="display: block;">
                                                                ...
                                                                <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iLocatioId']; ?>">
                                                                    <ul>
                                                                        <li class="entypo-twitter" data-network="twitter"><a href="location_wise_fare_action.php?id=<?= $data_drv[$i]['iLocatioId']; ?>" data-toggle="tooltip" title="Edit">
                    ...
                    <div class="admin-notes">
                            ...
                            <ul>
                                    <li>
                                            Locationwise Fare module will list all locations for flat fare on this page.
                                    </li>
                                    <li>
...
<form name="pageForm" id="pageForm" action="action/locationwise_fare.php" method="post" >
