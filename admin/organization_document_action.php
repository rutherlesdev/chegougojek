...
$sql = "select vCountry,vCompany from organization where iOrganizationId = '".$_REQUEST['id']."'";
$iOrganizationId = $obj->MySQLSelect($sql);

$script = 'Organization Document';
...
$sql1= "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status, dl.doc_masterid masterid_list ,dl.ex_date,dl.doc_file , dl.status, dm.eType FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='" .$_REQUEST['id']."' and doc_usertype='company') dl on dl.doc_masterid=dm.doc_masterid  
    where dm.doc_usertype='company' and dm.status='Active' and (dm.country ='".$iOrganizationId[0]['vCountry']."' OR dm.country ='All') $eTypeQuery";


$iCompanyIddoc = $obj->MySQLSelect($sql1);
$count_all = count($iCompanyIddoc);

$vName = $iOrganizationId[0]['vCompany'];
...
if ($action='document' && isset($_POST['doc_type'])) {
    ...
    if ($image_name == "") {
        ...
        if($expDate != ""){
			...
			header("location:organization_document_action.php?success=1&id=".$_REQUEST['id']."&var_msg=" . $var_msg);
			exit;
        }
         $var_msg = "Please upload valid file format for Image. Valid formats are pdf,doc,docx,jpg,jpeg,gif,png";
         header("location:organization_document_action.php?success=3&id=" . $_REQUEST['id'] . "&var_msg=" . $var_msg);
        exit;
    }
...
*/
    ...
    // });
	 ...
	 function setModel001(idVal,ex_status) {
			...
			$.ajax({
				type: 'post',
				url: 'organization_document_fetch.php', //Here you will fetch records
                cache: false,
                async: true,
