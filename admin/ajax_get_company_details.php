...
$sql="set @a=0,@b=0,@c=0;SELECT tr.iTripId AS TotalTrips, tr.iActive,(case when tr.iActive='Finished' then @a:=@a+1 when tr.iActive='Canceled' then @b:=@b+1 when tr.iActive='Active' then @c:=@c+1 END) as tot,@a as Finished,@b as Canceled,@c as Active FROM company cmp LEFT JOIN register_driver rd ON cmp.iCompanyId = rd.iCompanyId LEFT JOIN trips tr ON rd.iDriverId = tr.iDriverId WHERE cmp.iCompanyId =  '10' ";
