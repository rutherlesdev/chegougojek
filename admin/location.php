...
if ($keyword != '') {
    if ($option != '') {		
        if($eStatus != ''){
            $ssql.= " AND ".stripslashes($option)." LIKE '%".$generalobjAdmin->clean($keyword)."%' AND lm.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        }else {
            $ssql.= " AND ".stripslashes($option)." LIKE '%".$generalobjAdmin->clean($keyword)."%' AND lm.eStatus != 'Deleted'";
        }
    } else {
        if($eStatus != ''){
			$ssql .= " AND (lm.vLocationName LIKE '%" . $keyword . "%' OR lm.eFor LIKE '%" . $keyword . "%'  OR c.vCountry LIKE '%" . $keyword . "%')  AND lm.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
        ...
        if($eStatus != ''){
			$ssql .= " AND " . stripslashes($option) . " LIKE '" . $generalobjAdmin->clean($eFor) . "' AND lm.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
		}else{
			$ssql .= " AND " . stripslashes($option) . " LIKE '" . $generalobjAdmin->clean($eFor) . "' AND lm.eStatus != 'Deleted'";
		}
}else{
	 if($eStatus != ''){
		  $ssql.= " AND lm.eStatus = '".$generalobjAdmin->clean($eStatus)."'";
	 }else{
                            ...
                            <tbody>
								...
								<td width="12%" class="estatus_eFor" id="estatus_eFor" >
                                    ...
                                    <?php if ($userObj->hasPermission('create-geo-fence-locations')) { ?>
                                        <td width="20%"><a class="add-btn" href="location_action.php" style="text-align: center;">Add Location</a></td>
                                    <?php } ?>
                                </tr>
                            </tbody>
                        </table>
                                        ...
                                        <table class="table table-striped table-bordered table-hover">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th align="center"><a href="javascript:void(0);" onClick="Redirect(3,<?php
                                                                              if ($sortby == '3') {
                                                                                  echo $order;
                                                                              } else {
                                                                    ...
                                                                    <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iLocationId']; ?>">
                                                                        <ul>
                                                                            ...
                                                                            <?php if ($data_drv[$i]['eDefault'] != 'Yes') { ?>
                                                                                <?php if ($userObj->hasPermission('update-status-geo-fence-locations')) { ?>
                                                                                    <li class="entypo-facebook" data-network="facebook"><a href="javascript:void(0);" onclick="changeStatus('<?php echo $data_drv[$i]['iLocationId']; ?>', 'Inactive')"  data-toggle="tooltip" title="Activate">
                                                                                            <img src="img/active-icon.png" alt="<?php echo $data_drv[$i]['eStatus']; ?>" >
                                                                                        </a></li>
                                                                                    <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onclick="changeStatus('<?php echo $data_drv[$i]['iLocationId']; ?>', 'Active')" data-toggle="tooltip" title="Deactivate">
