...
if ($keyword != '') {
    ...
    if ($option != '') {
        ...
        if ($eStatus != '') {
            ...
            if (SITE_TYPE == 'Demo') {
                $ssql .= " AND (c.vCompany LIKE '%" . $generalobjAdmin->clean($keyword_new) . "%' OR c.vEmail LIKE '%" . $generalobjAdmin->clean($keyword_new) . "%' OR (concat(c.vCode,'',c.vPhone) = '" . $generalobjAdmin->clean($keyword_new) . "') ".$iUserProfileMasterIdIn.") AND c.eStatus = '" . $generalobjAdmin->clean($eStatus) . "'";
            }
        } else {
            ...
            if (SITE_TYPE == 'Demo') {
                $ssql .= " AND (c.vCompany LIKE '%" . $generalobjAdmin->clean($keyword_new) . "%' OR c.vEmail LIKE '%" . $generalobjAdmin->clean($keyword_new) . "%' OR (concat(c.vCode,'',c.vPhone) = '" . $generalobjAdmin->clean($keyword_new) . "') ".$iUserProfileMasterIdIn.")";
            }
        }
    }
