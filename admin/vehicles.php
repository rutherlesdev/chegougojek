...
<html lang="en">
    ...
    <head>
        <meta charset="UTF-8" />
        <title><?= $SITE_NAME ?> | <?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?> Vehicles</title>
                ...
                <div class="inner">
                    <div id="add-hide-show-div">
                        <div class="row">
                            <div class="col-lg-12">
<?
$drv_text = ($drv_name != "") ? "Vehicles of " . $generalobjAdmin->clearName($drv_name) : $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN'];
?>
...
<?php include('valid_msg.php'); ?>
    ...
    echo "selected";
                                            ...
                                            <? if (ONLYDELIVERALL != 'Yes') { ?>
                                                ...
                                                echo "selected";
                                            } ?> ><? if (isStoreDriverAvailable()>0) { ?>Company/Store<? } else { ?>Company <? } ?></option>
                                            <? } ?>
                                            <option value="" <?php if ($option == "CONCAT(rd.vName,' ',rd.vLastName)" || ($iDriverId != "" && $drv_name != "")) {
                                                echo "selected";
...
<? } ?>
    ...
    echo "selected";
                                    ...
                                    <td width="15%" class="searchform"><input type="Text" id="keyword" name="keyword" value="<?php echo $generalobjAdmin->clearName($keyword); ?>"  class="form-control" /></td>

                                    <td width="12%" class="eType_options" id="eType_options" >
                                        <select name="eType" id="eType_value" class="form-control">
                                            <option value="" >Select Status</option>
                                            <? if(isRideModuleAvailable()) { ?><option value='Ride' <?php if ($eType == 'Ride') {
    echo "selected";
} ?> >Ride</option><? } if(isDeliveryModuleAvailable()) { ?>
    ...
    echo "selected";
                                    ...
                                    <td>
                                        ...
                                        <input type="button" value="Reset" class="btnalt button11" onClick="window.location.href = 'vehicles.php'"/>
                                                ...
                                                <?php if ($userObj->hasPermission('create-provider-taxis')) { ?>
...
<?php if ($userObj->hasPermission(['update-status-provider-taxis', 'delete-provider-taxis'])) { ?>
                                                <select name="changeStatus" id="changeStatus" class="form-control" onchange="ChangeStatusAll(this.value);">
                                                    <option value="" >Select Action</option>
                                        <?php if ($userObj->hasPermission('update-status-provider-taxis')) { ?>
										<option value="Active"
    ...
    <?php } ?>
                                                        <?php if ($eStatus != 'Deleted' && $userObj->hasPermission('delete-provider-taxis')) { ?>
...
<?php } ?>
    ...
    echo $order;
                                                ...
                                                <? if (ONLYDELIVERALL != 'Yes') { ?>
                                                    ...
                                                    echo $order;
                                                } else { ?>0<?php } ?>)"><? if (isStoreDriverAvailable()>0) { ?>Company/Store<? } else { ?>Company <? } ?> <?php if ($sortby == 2) {
                                                    if ($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php }
                                        } else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                <? } ?>
...
<? //}  ?>
                                                    ...
                                                    <th width="8%" class="align-center"><a href="javascript:void(0);" onClick="Redirect(5,<?php if ($sortby == '5') {
    echo $order;
...
if (!empty($data_drv)) {
    for ($i = 0; $i < count($data_drv); $i++) {
        ...
        <? } ?>
                                                            <td><?= $generalobjAdmin->clearName($data_drv[$i]['driverName']); ?></td>
                                                                    ...
                                                                    <td align="center" width="12%">
                                                                        <a href="vehicle_document_action.php?id=<?= $data_drv[$i]['iDriverVehicleId']; ?>&vehicle=<?= $data_drv[$i]['vMake'] ?>" data-toggle="tooltip" title="Edit <?= $langage_lbl_admin['LBL_Vehicle']; ?> Document" target="_blank">
        ...
        <?php } ?>
        <?php if ($eStatus != 'Deleted' && $userObj->hasPermission('delete-provider-taxis')) { ?>
                                                                                <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onclick="changeStatusDeletevehicleCustom('<?php echo $data_drv[$i]['iDriverVehicleId']; ?>', '<?php echo $data_drv[$i]['iDriverId']; ?>','<?php echo $data_drv[$i]['tSessionId']; ?>')" data-toggle="tooltip" title="Delete">
...
<div data-backdrop="static" data-keyboard="false" class="modal fade" id="delete_driver_vehicle" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body"><p><?= $langage_lbl_admin['LBL_ACTIVE_VEHICLE_NOT_DELETE'] ?></p></div>
...
<?php include_once('footer.php'); ?>
        <script>
            ...
            function changeStatusDeletevehicleCustom(drivervehicleid,driverid,sessionid) {
                ...
                $.ajax({
                        ...
                        dataType: "json",
                        async: false,
                        data: {drivervehicleid: drivervehicleid, type: 'getDriverDetail', DriverAutoId: driverid,tSessionId: sessionid,GeneralMemberId: driverid,GeneralUserType: 'Driver'},
                        success: function (dataHtml2) {
                            if(dataHtml2!='' && dataHtml2.iDriverVehicleId!='') {
                                selectedDriverVehicle = dataHtml2.iDriverVehicleId;
                                return selectedDriverVehicle;
                            }
                        }
                });
                if(selectedDriverVehicle!='' && drivervehicleid==selectedDriverVehicle) {
