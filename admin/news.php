...
$searchusertype = isset($_REQUEST['searchusertype']) ? $_REQUEST['searchusertype'] : "";
$ssql = '';
if($keyword != ''){ 
    $keyword_new = $keyword;
    $chracters = array("(", "+", ")");
    ...
    if($option != '') {
		...
		if ($eStatus != '') {
            $ssql .= " AND (vTitle LIKE '%".$keyword_new."%'  OR eUserType LIKE '%".$keyword_new."%' ) AND eStatus = '" . $generalobjAdmin->clean($eStatus) . "'";
        } else {
            $ssql .= " AND (vTitle LIKE '%".$keyword_new."%'  OR eUserType LIKE '%".$keyword_new."%' )";
        }
       
    }
} else if ($eStatus != '' && $keyword == '' && $searchusertype == '') {

    $ssql .= " AND eStatus = '" . $generalobjAdmin->clean($eStatus) . "'";
}else if ($searchusertype != '' && $eStatus != '' && $keyword == '') {

    $ssql .= " AND eUserType = '" . $generalobjAdmin->clean($searchusertype) . "' AND eStatus = '" . $generalobjAdmin->clean($eStatus) . "'";
}else if ($searchusertype != '' && $eStatus == '' && $keyword == '') {
	
    $ssql .= " AND eUserType = '" . $generalobjAdmin->clean($searchusertype) . "'";
}
        ...
        <div id="wrap">
                              ...
                              <tbody>
                                <tr>
                                    ...
                                    <td width="10%" class=" padding-right10"><select name="option" id="option" class="form-control">
                                          ...
                                          <option  value="eUserType" <?php if ($option == "eUserType") { echo "selected"; } ?> >User Type</option> 
                                    </select>
                                    </td>
									...
									<td width="15%" class="usertype_options" id="usertype_options" >
                                        <select class="form-control" name ="searchusertype" id="searchusertype" required="required">
										   <option value="">Select User Type</option>
                                            <option value="all"  <?php if($searchusertype == 'all'){ echo 'selected'; } ?>>All</option>
                                            <option value="driver" <?php if($searchusertype == 'driver'){ echo 'selected'; } ?>><?php echo $langage_lbl_admin['LBL_DRIVER_TXT_ADMIN']; ?></option>
                                            <option value="rider" <?php if($searchusertype == 'rider'){ echo 'selected'; } ?>><?php echo $langage_lbl_admin['LBL_RIDER']; ?></option>
                                            <option value="company" <?php if($searchusertype == 'company'){ echo 'selected'; } ?>>Store/ Restaurant</option>
			...
			<td width="12%" class="estatus_options" id="eStatus_options" >
									...
									<td width="20%">
                                      ...
                                      <input type="button" value="Reset" class="btnalt button11" onClick="window.location.href='news.php'"/>
                                    </td>
                                    <?php   if($userObj->hasPermission('create-news')){ ?>
                                        <td width="30%"><a class="add-btn" href="news_action.php" style="text-align: center;">Add news</a></td>
                                    <?php   } ?>
                                </tr>
                              </tbody>
                        </table>
                                    ...
                                    <span class="col-lg-2 new-select001">
                                        <?php  if($userObj->hasPermission(['update-status-news', 'delete-news'])){ ?>
                                            <select name="changeStatus" id="changeStatus" class="form-control" onchange="ChangeStatusAll(this.value);">
                                                ...
                                                <?php  } ?>
                                                <?php  if($eStatus != 'Deleted' && $userObj->hasPermission('delete-news')) { ?>
                                            ...
                                            <table class="table table-striped table-bordered table-hover">
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th width="20%"><a href="javascript:void(0);" onClick="Redirect(2,<?php if($sortby == '2'){ echo $order; }else { ?>0<?php } ?>)">Published date <?php if ($sortby == 2) { if($order == 0) { ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php } }else { ?><i class="fa fa-sort" aria-hidden="true"></i> <?php } ?></a></th>
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr class="gradeA">
                                                        <td align="center" style="text-align:center;"><input type="checkbox" id="checkbox" name="checkbox[]" <?php echo $default; ?> value="<?php echo $data_drv[$i]['iNewsfeedId']; ?>" />&nbsp;</td>
                                                        <td><?php
                                                        $newsTitle = '';
                                                        ...
                                                        <td><?=  $generalobjAdmin->DateTime($data_drv[$i]['tPublishdate'], 'No')  ?></td>
                                                        ...
                                                        <td align="center" style="text-align:center;" class="action-btn001">
                                                                <div class="share-button openHoverAction-class" style="display: block;">
                                                                    ...
                                                                    <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iNewsfeedId']; ?>">
                                                                        <ul>
                                                                            <li class="entypo-twitter" data-network="twitter"><a href="news_action.php?id=<?= $data_drv[$i]['iNewsfeedId']; ?>" data-toggle="tooltip" title="Edit">
                                                                                <img src="img/edit-icon.png" alt="Edit">
                                                                            </a></li>
                                                                            <?php  if($userObj->hasPermission('update-status-news')){ ?>
                                                                                    <li class="entypo-facebook" data-network="facebook"><a href="javascript:void(0);" onclick="changeStatus('<?php echo $data_drv[$i]['iNewsfeedId']; ?>','Inactive')"  data-toggle="tooltip" title="Activate">
                                                                                        <img src="img/active-icon.png" alt="<?php echo $data_drv[$i]['eStatus']; ?>" >
                                                                                    </a></li>
                                                                                    <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onclick="changeStatus('<?php echo $data_drv[$i]['iNewsfeedId']; ?>','Active')" data-toggle="tooltip" title="Deactivate">
                                                                                        <img src="img/inactive-icon.png" alt="<?php echo $data_drv[$i]['eStatus']; ?>" >    
                                                                                    </a></li>
                                                                            <?php  } ?>
                                                                            <?php if($userObj->hasPermission('delete-news')){ ?>
                                                                                    <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onclick="changeStatusDelete('<?php echo $data_drv[$i]['iNewsfeedId']; ?>')"  data-toggle="tooltip" title="Delete">
