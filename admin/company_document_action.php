...
$sql1= "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status, dl.doc_masterid masterid_list ,dl.ex_date,dl.doc_file ,dl.req_date,dl.doc_id,dl.req_file,  dl.status, dm.eType FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='" .$_REQUEST['id']."' and doc_usertype='company') dl on dl.doc_masterid=dm.doc_masterid  
    where dm.doc_usertype='company' and dm.status='Active' and (dm.country ='".$iCompanyId[0]['vCountry']."' OR dm.country ='All') $eTypeQuery";
...
$sql2= "SELECT dm.doc_masterid masterid, dm.doc_usertype , dm.doc_name ,dm.ex_status,dm.status, dl.doc_masterid masterid_list ,dl.ex_date,dl.doc_file ,dl.req_date,dl.doc_id,dl.req_file,  dl.status, dm.eType FROM document_master dm left join (SELECT * FROM `document_list` where doc_userid='" .$_REQUEST['id']."' and doc_usertype='company') dl on dl.doc_masterid=dm.doc_masterid  
   where dl.req_date != '' AND dl.req_date != '0000-00-00' and dm.doc_usertype='company' and dm.status='Active' and (dm.country ='".$iCompanyId[0]['vCountry']."' OR dm.country ='All') $eTypeQuery";
                                            ...
                                            <div class="panel-body">
                                                <?php if ($iCompanyIddoc[$i]['doc_file'] != '' && file_exists('../webimages/upload/documents/company/' . $_REQUEST['id'] . '/' . $db_userdoc[$i]['doc_file'])) { ?>
                                                    ...
                                                    if ($file_ext == 'is_image') {
                                                        $imgpath = $tconfig["tsite_upload_compnay_doc"] . '/' . $_REQUEST['id'] . '/' . $iCompanyIddoc[$i]['doc_file'];
                                ...
                                <?php for ($i = 0; $i < $count_all2; $i++) { ?>
                                        ...
                                        <div class="panel panel-default upload-clicking">
                                            ...
                                            <div class="panel-body" style="display: inline-block;">
                                                <?php if ($iCompanyIddoc2[$i]['req_file'] != '' && file_exists('../webimages/upload/documents/company/' . $_REQUEST['id'] . '/' . $db_userdoc[$i]['req_file'])) { ?>
                                                    <?php
                                                    $file_ext = $generalobj->file_ext($iCompanyIddoc2[$i]['req_file']);
                                                    if ($file_ext == 'is_image') {
                                                        $imgpath1 = $tconfig["tsite_upload_compnay_doc"] . '/' . $_REQUEST['id'] . '/' . $iCompanyIddoc2[$i]['req_file'];
                                                        $resizeimgpath1 = $tconfig['tsite_url'] . "resizeImg.php?src=" . $imgpath1 . "&w=200";
                                                        ?>
                                                        <a href="<?= $tconfig["tsite_upload_compnay_doc"] . '/' . $_REQUEST['id'] . '/' . $iCompanyIddoc2[$i]['req_file'] ?>" target="_blank"><img src = "<?= $resizeimgpath1; ?>" style="cursor:pointer;" alt ="YOUR DRIVING LICENCE" /></a>
                                                        <!-- data-toggle="modal" data-target="#myModallicence" -->
                                                    <?php } else { ?>
                                                        <p><a href="<?= $tconfig["tsite_upload_compnay_doc"] . '/' . $_REQUEST['id'] . '/' . $iCompanyIddoc2[$i]['req_file'] ?>" target="_blank"><?php echo $iCompanyIddoc2[$i]['doc_name']; ?></a></p>
                                                    <?php } ?>
                                                    <?php
                                                } else {
...
$(document).on('click', '#approveDoc', function(event) {
    ...
    var request = $.ajax({
        ...
        success: function (data)
        {
            window.location = 'company_document_action.php?success=4&id=<?php echo $_REQUEST['id']; ?>';
        }
    });
