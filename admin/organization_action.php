...
if (isset($_POST['submit'])) {
    ...
    /* if ($vEmail != "") {
      if ($id != "") {
      $msg1 = $generalobj->checkDuplicateAdminNew('iOrganizationId','organization', Array('vEmail'), $id, "");
      } else {
      $msg1 = $generalobj->checkDuplicateAdminNew('vEmail', 'organization', Array('vEmail'), "", "");
      }
      if ($msg1 == 1) {
    ...
    $eSystem = "";
    $checEmailExist = $generalobj->checkMemberDataInfo($vEmail, "", 'organization', $vCountry, $id, $eSystem);
    if ($checEmailExist['status'] == 0) {
        $error .= '* Email Address is already exists.<br>';
    } else if ($checEmailExist['status'] == 2) {
        $error .= $langage_lbl['LBL_INVALID_MEMBER_USER_COUNTRY_EMAIL_TXT'];
    }
    $checPhoneExist = $generalobj->checkMemberDataInfo($vPhone, "", 'organization', $vCountry, $id, $eSystem);
