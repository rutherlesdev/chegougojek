...
if (($statusVal == 'Deleted' || $method == 'delete') && ($iLocatioId != '' || $checkbox != "")) {
    if (!$userObj->hasPermission('delete-location-wise-fare')) {
        ...
        if (SITE_TYPE != 'Demo') {
            // $query = "DELETE FROM airportsurcharge_fare WHERE iLocatioId IN (" . $locationIds . ")";
            $query = "UPDATE airportsurcharge_fare SET eStatus = 'Deleted' WHERE iLocatioId IN (" . $locationIds . ")";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
    ...
    header("Location:" . $tconfig["tsite_url_main_admin"] . "airport_surcharge.php?" . $parameters);
    exit;
}
//End make deleted
...
if ($iLocatioId != '' && $status != '') {
    if (!$userObj->hasPermission('update-status-location-wise-fare')) {
        ...
        if (SITE_TYPE != 'Demo') {
            $query = "UPDATE airportsurcharge_fare SET eStatus = '" . $status . "' WHERE iLocatioId = '" . $iLocatioId . "'";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
            if ($status == 'Active') {
...
if ($checkbox != "" && $statusVal != "") {
    if (!$userObj->hasPermission('update-status-location-wise-fare')) {
        ...
        if (SITE_TYPE != 'Demo') {
            $query = "UPDATE airportsurcharge_fare SET eStatus = '" . $statusVal . "' WHERE iLocatioId IN (" . $checkbox . ")";
            $obj->sql_query($query);
            $_SESSION['success'] = '1';
