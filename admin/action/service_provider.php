...
if ($method == 'delete' && $iDriverId != '') {
	...
	header("Location:".$tconfig["tsite_url_main_admin"]."service_provider.php?".$parameters); exit;
}
//End make deleted
...
if ($iDriverId != '' && $status != '') {
	if(SITE_TYPE !='Demo'){
        ...
        header("Location:".$tconfig["tsite_url_main_admin"]."service_provider.php?".$parameters);
        exit;
}
