...
if(!empty($iServiceId)){
	$sql = "SELECT c.iCompanyId,c.vCompany,c.iServiceId,c.vEmail FROM `company` AS c LEFT JOIN food_menu AS f ON c.iCompanyId = f.iCompanyId WHERE iServiceId = '".$iServiceId."' and  c.eStatus!='Deleted' GROUP BY c.iCompanyId ORDER BY `vCompany`";
