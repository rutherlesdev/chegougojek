...
if ($sortby == 5) {
    ...
    if ($order == 0)
        $ord = " ORDER BY dm.eDocServiceType ASC";
    else
...
if ($keyword != '') {
    ...
    if ($option != '') {
        if ($eType_value != '') {
            $ssql .= " AND " . stripslashes($option) . " LIKE '%" . stripslashes($keyword) . "%' AND dm.eDocServiceType = '" . $generalobjAdmin->clean($eType_value) . "'";
        } else {
        ...
        if ($eType_value != '') {
            $ssql .= " AND (c.vCountry LIKE '%" . $keyword . "%' OR dm.doc_usertype LIKE '%" . $keyword . "%' OR dm.doc_name LIKE '%" . $keyword . "%' OR dm.status LIKE '%" . $keyword . "%') AND dm.eDocServiceType = '" . $generalobjAdmin->clean($eType_value) . "'";
        } else {
            $ssql .= " AND (c.vCountry LIKE '%" . $keyword . "%' OR dm.doc_usertype LIKE '%" . $keyword . "%' OR dm.doc_name LIKE '%" . $keyword . "%' OR dm.status LIKE '%" . $keyword . "%')";
        }

        if ($doc_userTypeValue != '') {
            $ssql .= " AND (c.vCountry LIKE '%" . $keyword . "%' OR dm.doc_name LIKE '%" . $keyword . "%' OR dm.status LIKE '%" . $keyword . "%') AND dm.eDocServiceType = '" . $generalobjAdmin->clean($eType_value) . "' AND dm.doc_usertype = '" . $generalobjAdmin->clean($doc_userTypeValue) . "'";
        } else {
            $ssql .= " AND (c.vCountry LIKE '%" . $keyword . "%' OR dm.doc_usertype LIKE '%" . $keyword . "%' OR dm.doc_name LIKE '%" . $keyword . "%' OR dm.status LIKE '%" . $keyword . "%')";
        }
    }
} else if ($eType_value != '' && $keyword == '') {
    $ssql .= " AND dm.eDocServiceType = '" . $generalobjAdmin->clean($eType_value) . "'";
...
$sql = "SELECT dm.doc_masterid, dm.doc_usertype , dm.doc_name, dm.status , c.vCountry, dm.eDocServiceType FROM `document_master` AS dm LEFT JOIN `country` AS c ON c.vCountryCode=dm.country WHERE 1=1 $eStatussql $ssql $ord LIMIT $start, $per_page";
                        ...
                        <table width="100%" border="0" cellpadding="0" cellspacing="0" class="admin-nir-table">
                            <tbody>
                                <tr>
                                    ...
                                    <td width="12%" class="doc_userType" id="doc_userType" >
                                        ...
                                        <table class="table table-striped table-bordered table-hover">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th width="25%"><a href="javascript:void(0);" onClick="Redirect(3,<?php
                                                        if ($sortby == '3') {
                                                            echo $order;
                                                        } else {
                                                    ...
                                                    <!-- <? if ($APP_TYPE == 'Ride-Delivery' || $APP_TYPE == 'Ride-Delivery-UberX') { ?>
                                                                <th width="8%"><a href="javascript:void(0);" onClick="Redirect(5,<?php
                                                        if ($sortby == '5') {
                                                            echo $order;
                                                        } else {
                                                            ?>0<?php } ?>)">Service Type <?php
                                                        if ($sortby == 5) {
                                                            if ($order == 0) {
                                                                ?><i class="fa fa-sort-amount-asc" aria-hidden="true"></i> <?php } else { ?><i class="fa fa-sort-amount-desc" aria-hidden="true"></i><?php
                                                                }
    ...
    for ($i = 0; $i < count($data_drv); $i++) {
        ?>
                                                        <tr class="gradeA">
                                                            ...
                                                            <!--    <?
                                                            if ($APP_TYPE == 'Ride-Delivery' || $APP_TYPE == 'Ride-Delivery-UberX') {
                                                                if ($data_drv[$i]['eDocServiceType'] == 'UberX') {
                                                                    $etype = 'Other Services';
                                                                } else {
                                                                    if ($data_drv[$i]['doc_usertype'] == 'company') {
                                                                        $etype = '---';
                                                                    } else {
                                                                        $etype = $data_drv[$i]['eDocServiceType'];
                                                                    }
                                                                }
                                                                ?>
                                                                             <td><?= $etype; ?></td>
        ...
        <?php if ($data_drv[$i]['eDefault'] != 'Yes') { ?>
            <?php if ($userObj->hasPermission('update-status-documents')) { ?>
                                                                                    <li class="entypo-facebook" data-network="facebook"><a href="javascript:void(0);" onclick="changeStatus('<?php echo $data_drv[$i]['doc_masterid']; ?>', 'Inactive')"  data-toggle="tooltip" title="Activate">
                                                                                            <img src="img/active-icon.png" alt="<?php echo $data_drv[$i]['eStatus']; ?>" >
                                                                                        </a></li>
                                                                                    <li class="entypo-gplus" data-network="gplus"><a href="javascript:void(0);" onclick="changeStatus('<?php echo $data_drv[$i]['doc_masterid']; ?>', 'Active')" data-toggle="tooltip" title="Deactivate">
