...
//echo TPATH_CLASS . "class.general_admin.php";
...
if ($keyword != '') {
    ...
    if ($option != '') {
        ...
        if ($eStatus != '') {
            ...
            if (SITE_TYPE == 'Demo') {
                $ssql .= " AND " . stripslashes($option_new) . " = '" . $generalobjAdmin->clean($keyword_new) . "' AND c.eStatus = '" . $generalobjAdmin->clean($eStatus) . "'";
            }
        } else {
...
$sql = "SELECT COUNT(iCompanyId) AS Total FROM company AS c WHERE 1 = 1 $eSystem $eStatus_sql $ssql $cmp_ssql";
...
$sql = "SELECT c.iCompanyId, c.vCompany, c.vEmail,(SELECT count(rd.iDriverId) FROM register_driver AS rd WHERE rd.iCompanyId=c.iCompanyId AND rd.eStatus != 'Deleted' $dri_ssql) AS `count`, c.vCode,c.vPhone, c.eStatus FROM company AS c WHERE 1 = 1 $eSystem $equery $ssql $cmp_ssql $ord LIMIT $start, $per_page";
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <div class="admin-nir-export">
                                    ...
                                    ?> >Deactivate</option>
                                        <?php } ?>
    <?php if ($eStatus != 'Deleted' && $userObj->hasPermission('delete-company')) { ?>
                                                                                                        <option value="Deleted" <?php
        if ($option == 'Delete') {
            echo "selected";
        }
                                                                    ...
                                                                    <div class="share-button share-button4 openHoverAction-class" style="display:block;">
                                                                        ...
                                                                        <div class="social show-moreOptions openPops_<?= $data_drv[$i]['iCompanyId']; ?>">
                                                                            <ul>
                                                                                ...
                                                                                <?php if ($userObj->hasPermission('update-status-company')) { ?>
                                                                                    <li class="entypo-facebook" data-network="facebook"><a href="javascript:void(0);" onClick="changeStatus('<?php echo $data_drv[$i]['iCompanyId']; ?>', 'Inactive')"  data-toggle="tooltip" title="Activate">
...
<?php if (strtoupper($ufxService) == "YES") { ?>
                                <li>This module will list all the companies registered as a Taxi/Ride, Common Delivery, and Other Services.</li>
<?php } else { ?>
