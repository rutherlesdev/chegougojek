...
if ($iCompanyId != '') {
    ...
    for ($i = 0; $i < count($db_model); $i++) {
        if ($db_model[$i]['iDriverId'] == $iDriverId)
            $cont .= '<option value="' . $db_model[$i]['iDriverId'] . '"  ' . $selected . '>' . $generalobjAdmin->clearName($db_model[$i]['vName'] . ' ' . $db_model[$i]['vLastName']) . ' ('. $generalobjAdmin->clearEmail($db_model[$i]['vEmail']) .')</option>';
        else
            $cont .= '<option value="' . $db_model[$i]['iDriverId'] . '">' . $generalobjAdmin->clearName($db_model[$i]['vName'] . ' ' . $db_model[$i]['vLastName']) . ' ('. $generalobjAdmin->clearEmail($db_model[$i]['vEmail']) .')</option>';
    }
}
$cont .= '</select>';
echo $cont;
