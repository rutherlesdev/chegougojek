                                    ...
                                    if (count($allservice_cat_data) > 1) {
                                        foreach ($allservice_cat_data as $langOpt) {
                                            ...
                                            if ($selectedlanguage == $langOpt['iServiceId']) {
                                                $name = $generalobjAdmin->clearName($langOpt['vServiceName']);
                                            }
                                        }
                                    }
                                } else {
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td width="10%" class=" padding-right10"><select name="option" id="option" class="form-control">
                                            ...
                                            ?> >Value In <?= $lang_title[0]['vTitle'] ?> Language</option>
                                        </select>
                                    </td>
                                    ...
                                    <td width="30%">
                                        ...
                                        if (!empty($_SERVER['HTTP_HOST']) && ($_SERVER['HTTP_HOST'] == "192.168.1.131" || $_SERVER['HTTP_HOST'] == "192.168.1.151")) {
                                        ?>
                                            <? if (empty($selectedlanguage)) { ?>
                                                 <a class="add-btn" href="languages_action_multisystem.php" style="text-align: center;">Add Label</a>
                                            <? } else { ?>
                                            <a class="add-btn" href="languages_action_multisystem_food_other.php" style="text-align: center;">Add Label</a>
                                        <?php }} else { ?>
                                            <?php if ($userObj->hasPermission('create-general-label')) { ?>
                                                <? if (!empty($selectedlanguage)) { ?>
                                                    <a class="add-btn" href="languages_action.php?selectedlanguage=<?= $selectedlanguage ?>" style="text-align: center;">Add Label</a>
                                                <?php } else { ?>
                                                    <a class="add-btn" href="languages_action.php" style="text-align: center;">Add Label</a>
                                                <? } ?>
                                            <?php } ?>
                                        <? } ?> 
                                    </td>
                                    ...
                                    <!-- <span class="col-lg-2 new-select001">
                                        ...
                                        <? if (count($allservice_cat_data) > 1) { ?>
                                            ...
                                            <? foreach ($allservice_cat_data as $langOpt) { ?>
                                                ...
                                                ?>><?php echo $generalobjAdmin->clearName($langOpt['vServiceName']); ?></option>
                                            <? } ?>
                                                                 </select>
                                                             </span>
                                        <? } else { ?>
                                                    ...
                                                    for ($i = 0; $i < count($data_drv); $i++) {
                                                        ...
                                                        <tr class="gradeA">
                                                            ...
                                                            <td align="center" style="text-align:center;" class="action-btn001">
                                                                ...
                                                                ?> 
                                                                    <? if (empty($selectedlanguage)) {  ?>
                                                                        <a href="languages_action_multisystem.php?id=<?= $data_drv[$i]['LanguageLabelId']; ?>" data-toggle="tooltip" title="Edit">
                                                                            <img src="img/edit-icon.png" alt="Edit">
                                                                        </a>
                                                                        <a href="javascript:void(0)" data-toggle="tooltip" title="Delete" onclick="delete_lang_label('<?= $data_drv[$i]['vLabel']; ?>')">
                                                                            <img src="img/delete-icon.png" alt="Delete">
                                                                        </a>
                                                                    <? } else { ?>
                                                                        <a href="languages_action_multisystem_food_other.php?id=<?= $data_drv[$i]['LanguageLabelId']; ?>" data-toggle="tooltip" title="Edit">
                                                                            <img src="img/edit-icon.png" alt="Edit">
                                                                        </a>
                                                                    <?php } ?>                                                            
                                                                <? } else { ?>      
                                                                    ...
                                                                    <? } else { ?>
                                                                        <a href="languages_action.php?id=<?= $data_drv[$i]['LanguageLabelId']; ?>" data-toggle="tooltip" title="Edit">
                                                                            <img src="img/edit-icon.png" alt="Edit">
                                                                        </a>
                                                                    <? } ?>
                                                                <? } ?>
...
//                alert(action+formValus);
            ...
            function delete_lang_label(vLabel) {
				...
				if (confirm_box == true) {
					...
					$.ajax({
						type: 'POST',
						url: 'ajax_delete_language_label.php',
						data: {'vLabel': vLabel},
						cache: false,
