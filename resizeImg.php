...
require_once('assets/libraries/imagecraft/autoload.php');
require_once('assets/libraries/gifImageResizer.php');
use Imagecraft\ImageBuilder;


if(isset($_REQUEST['IMG_MAX_HEIGHT']) && !empty($_REQUEST['IMG_MAX_HEIGHT']) && $_REQUEST['IMG_MAX_HEIGHT'] > 0){
    define('MAX_HEIGHT', $_REQUEST['IMG_MAX_HEIGHT']);
}
if(isset($_REQUEST['IMG_MAX_WIDTH']) && !empty($_REQUEST['IMG_MAX_WIDTH']) && $_REQUEST['IMG_MAX_WIDTH'] > 0){
    define('MAX_WIDTH', $_REQUEST['IMG_MAX_WIDTH']);
}
...
if (!defined('FILE_CACHE_TIME_BETWEEN_CLEANS'))
    define('FILE_CACHE_TIME_BETWEEN_CLEANS', 604800); // How often the cache is cleaned 

if (!defined('FILE_CACHE_MAX_FILE_AGE'))
    define('FILE_CACHE_MAX_FILE_AGE', 604800);    // How old does a file have to be to be deleted from the cache
...
if (!defined('FILE_CACHE_DIRECTORY'))
    define('FILE_CACHE_DIRECTORY', './webimages/resizeImage/cache');    // Directory where images are cached. Left blank it will use the system temporary directory (which is better for security)
if (!defined('FILE_FRAMES_DIRECTORY'))
    define('FILE_FRAMES_DIRECTORY', 'webimages/resizeImage/frames');
if (!defined('MAX_FILE_SIZE'))
    define('MAX_FILE_SIZE', 50485760);      // 10 Megs is 10485760. This is the max internal or external file size that we'll process.  
...
if (!defined('MAX_WIDTH'))
    define('MAX_WIDTH', 16000);        // Maximum image width
if (!defined('MAX_HEIGHT'))
    define('MAX_HEIGHT', 16000);       // Maximum image height
...
if (!defined('PNG_IS_TRANSPARENT'))
    define('PNG_IS_TRANSPARENT', TRUE);     // Define if a png image should have a transparent background color. Use False value if you want to display a custom coloured canvas_colour 
...
if (!defined('DEFAULT_WIDTH'))
    define('DEFAULT_WIDTH', 1024);       // Default thumbnail width. Allows overrid in timthumb-config.php
if (!defined('DEFAULT_HEIGHT'))
    define('DEFAULT_HEIGHT', 1024);       // Default thumbnail height. Allows overrid in timthumb-config.php
...
class timthumb {
    ...
    public function __construct() {
		...
		try{
			$imgFileInfo = new SplFileInfo($this->src);
		
			if(!empty($imgFileInfo->getExtension()) && ($imgFileInfo->getExtension() == "gif" || $imgFileInfo->getExtension() == "GIF")){
				$cache_dir_gif = FILE_CACHE_DIRECTORY;
				
				$localImageGIFPath = $cache_dir_gif."/".basename($this->src, ".".$imgFileInfo->getExtension())."_".$_REQUEST['w']."_".$_REQUEST['h'].".".$imgFileInfo->getExtension();
				
				if(file_exists($localImageGIFPath)){
					header('Content-Type: image/gif');
					$image= file_get_contents($localImageGIFPath);
					echo $image;

					exit;
				}
				
				$tmpDir = FILE_FRAMES_DIRECTORY;
				if (!is_dir($tmpDir)){
					mkdir($tmpDir, 0777);
				}
				
				$localPathOfSourceURL = $this->getLocalImagePath($this->src);
				
				$gr = new GifRImage();	//New Instance Of GIFResizer
				$gr->temp_dir = $tmpDir; //Used for extracting GIF Animation Frames
				$gr->resize($localPathOfSourceURL,$localImageGIFPath,$_REQUEST['w'],$_REQUEST['h']); //Resizing the animation into a new file.
				...
				$options = ['engine' => 'php_gd', 'locale' => 'en', 'gif_animation' => 'true', 'memory_limit' => MEMORY_LIMIT, 'cache_dir' => $cache_dir_gif, 'debug' => 'false'];
				$builder = new ImageBuilder($options);
				$image = $builder
					->addBackgroundLayer()
						->http($_REQUEST['src'], MAX_FILE_SIZE, 50)
						->resize($_REQUEST['w'], $_REQUEST['h'], 'shrink')
						->done()
					->save();
				if ($image->isValid()) {
					// echo "Path:".$localImage;exit;
					file_put_contents($localImageGIFPath, $image->getContents());
			...
			}else if(!empty($imgFileInfo->getExtension()) && ($imgFileInfo->getExtension() == "svg" || strtoupper($imgFileInfo->getExtension()) == "SVG")){
				$localPathOfSourceURL = $this->getLocalImagePath($this->src);
				header('Content-Type: image/svg+xml');
				$image= file_get_contents($localPathOfSourceURL);
				echo $image;
				exit;
			}
		}catch(Exception $e){
