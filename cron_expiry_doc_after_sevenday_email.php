...
.table-bordered1{ margin-bottom:20px;}
</style>
<body>
<?php
	error_reporting(0);
	...
	$ToDate2 = date('Y-m-d', strtotime("+15 days"));
	...
	WHERE doc.ex_date LIKE '%$ToDate%' OR doc.ex_date LIKE '%$ToDate2%'";
	...
	for($i=0;$i< count($data_ex_doc);$i++){
		...
		if($data_ex_doc[$i]['doc_usertype'] == "company"){
			...
			if(!empty($data_ex_doc[$i]['Cmpname'])){
				$cmplink = $tconfig["tsite_url"].''.ADMIN_URL_CLIENT.'/company_document_action.php?id='.$data_ex_doc[$i]['cmpid'].'&action=edit';
		...
		if($data_ex_doc[$i]['doc_usertype'] == "driver"){
			...
			$link = $tconfig["tsite_url"].''.ADMIN_URL_CLIENT.'/driver_document_action.php?id='.$data_ex_doc[$i]['driverid'].'&action=edit&user_type=driver';
	...
	if((count($array_driver_detail) > 0 )|| (count($array_cmp_detail) > 0)){
		...
		//echo $driver_message; exit;
		$message = array();
		$message['details'] =$driver_message;
		$mail = $generalobj->send_email_user('CRON_EXPIRY_DOCUMENT_EMAIL_SEVEN_DAY_AGO',$message);	 
	}
