...
$user=$id[2];
if($user == 'company'){
	$sql = "select  dm.`doc_masterid`, dm.`doc_usertype`, dm.`doc_name`, dm.doc_name_".$_SESSION['sess_lang']." as document ,c.eStatus as eStatus, dm.`ex_status`, dl.`doc_id`, dl.`doc_masterid`, dl.`doc_usertype`, dl.`doc_userid`, dl.`ex_date`, dl.`doc_file`,dl.`req_date`,dl.`req_file`,c.`iCompanyId`,count(usl.iUserId) as statusUpdatedCount from document_master as dm left join document_list  as dl on dl.doc_masterid= dm.doc_masterid left join  company as c on  dl.doc_userid= c.iCompanyId left join user_status_logs as usl ON usl.iUserId = c.iCompanyId where c.iCompanyId='".$id[1]."' and dl.doc_usertype='company' and dm.doc_masterid='".$id[0]."' group by usl.iUserId" ;
		
} else {
	$sql = "select  dm.`doc_masterid`, dm.`doc_usertype`, dm.`doc_name`, dm.doc_name_".$_SESSION['sess_lang']." as document,rd.eStatus as eStatus, dm.`ex_status`, dl.`doc_id`, dl.`doc_masterid`, dl.`doc_usertype`, dl.`doc_userid`, dl.`ex_date`, dl.`doc_file`,dl.`req_date`,dl.`req_file`,rd.`iDriverId`,count(usl.iUserId) as statusUpdatedCount from document_master as dm left join document_list  as dl on  dl.doc_masterid= dm.doc_masterid left join  register_driver as rd on  dl.doc_userid= rd.iDriverId left join user_status_logs as usl ON usl.iUserId = dl.doc_userid where dl.doc_usertype='driver' AND  iDriverId='".$id[1]."' and dm.doc_masterid='".$id[0]."' group by usl.iUserId" ;	
}	
    ...
    <form class="form-horizontal frm6" id="frm6" method="post" enctype="multipart/form-data" action="profile.php?id=<?php echo $id[1] ; ?>&master=<?php echo $id[0] ; ?> " name="frm6">
        ...
        <div class="col-lg-13 exp-date">
            <div class="input-group input-append date" id="dp122">
                ...
                <?php 
                // echo $db_user[0]['eStatus'] .' - '. $db_user[0]['statusUpdatedCount'];
                $disable = 'disabled';
